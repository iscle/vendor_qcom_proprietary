...
#define DYNAMIC_HEAP_SIZE 0x2800000//40MB secure memory added to amss heap in securePD.
#define HLOS_PHYSPOOL_SIZE 0x800000 //8MB of secure memory added to HLOS Physical Pool in securePD.
...
static int initialize_securepd(keymaster_handle_t* km_handle) {
    ...
    if (nErr) {
        ALOGE("%s: Failed to allocate heap buffer of size:%d\n", __func__, DYNAMIC_HEAP_SIZE);
        goto bail;
    }
    ...
    printf("%s: allocated and registered secure POOL fd %d of size %d\n", __func__, poolHandle.fd, HLOS_PHYSPOOL_SIZE);
...
int main(int argc, char *argv[])
{
    ...
    while ((command = getopt_long(argc, argv, "l:r:e:h:s", example_options, NULL)) != -1) {
        switch (command) {
            case 'l':
                while (1) {
