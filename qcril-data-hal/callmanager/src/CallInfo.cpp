  ...
  if((!pProfileInfo->getApn().empty()) || (pProfileInfo->getApn().empty() && DATA_IS_RIL_RADIO_TECH_3GPP(pProfileInfo->getAvailableRadioRAT()))) {
    ...
    if ((pProfileInfo->getCdmaProfileId() != QDP_PROFILE_ID_INVALID ||
      ...
      if(mCallBringupCapability & BRING_UP_APN_TYPE) {
        ...
        wdsApnTypeMask =
          WDSModemEndPointModule::convertToApnTypeMask((int32_t)pProfileInfo->getApnTypes());
        ...
        logBuffer.addLogWithTimestamp("[CallInfo(" + std::to_string(cId) + ")]: Bringup using" +
          " apn type mask=" + std::to_string((int)wdsApnTypeMask));

      } else { /* profile ID based bringup */
