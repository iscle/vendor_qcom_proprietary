...
public class ImsServiceSub extends MmTelFeature
    ...
    //    mCi.getImsRegistrationState(mHandler.obtainMessage(EVENT_IMS_STATE_DONE));
        ...
        if (mSubscriptionManager != null) {
            ...
            loge("unable to listen for subscription changed due to " +
    ...
    private final SubscriptionManager.OnSubscriptionsChangedListener mOnSubscriptionsChangeListener
        ...
        public void onSubscriptionsChanged() {
            final SubscriptionInfo subInfo =
                    mSubscriptionManager.getActiveSubscriptionInfoForSimSlotIndex(mPhoneId);
            if (subInfo == null) {
                loge("onSubscriptionsChanged unable to process due to " +
                        "SubscriptionInfo is null");
                return;
            }

            final int subId = subInfo.getSubscriptionId();
            logv("onSubscriptionsChanged oldSubId = " + mSubId + " new SubId " + subId);
            if (mSubId != subId) {
                //Clean listeners for old subscription
                mTm.listen(mPhoneStateListener, PhoneStateListener.LISTEN_NONE);
                mSubId = subId;
                mTm = mTm.createForSubscriptionId(subId);
                //Register for SRVCC state indications on CS PIPE
                mTm.listen(mPhoneStateListener, PhoneStateListener.LISTEN_SRVCC_STATE_CHANGED);
            }
        }
    };
    ...
    private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        public void onSrvccStateChanged(int srvccState) {
            if (mTracker != null) {
                mTracker.calculateOverallSrvccState(srvccState);
            } else {
                loge("Error onSrvccStateChanged tracker is null");
            }
        }
    };
