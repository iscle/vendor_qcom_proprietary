...
int32_t Acdb_GetInstanceTblEntries(uint8_t *pCmd, uint32_t nCmdSize, uint8_t *pRsp, size_t nRspSizse)
{
   ...
   if (nonModuleTblFound == 0)
   {
      ...
      {
         AcdbInstanceVoiceTblType *pCur;
         noOfBytesReqPerEntry = ((noOfTableIndices + 2) * sizeof(uint32_t)); //2 is added to include cdftPtr and pid
                     ...
                     while (pCurTop != NULL)
                     {
                        if (pInput->nTblEntriesOffset <= curNoOfEntriesOffset)
                        {
                           if (((pInput->nBuffSize - offset) >= noOfBytesReqPerEntry))
                           {
                              ...
                              offset += ((noOfCmdIndices)*sizeof(uint32_t));
                              ACDB_MEM_CPY(pInput->pBuff + offset, ((noOfTableIndices - noOfCmdIndices)*sizeof(uint32_t)), pLut, (noOfTableIndices - noOfCmdIndices)*sizeof(uint32_t));
                              offset += ((noOfTableIndices - noOfCmdIndices)*sizeof(uint32_t));
							  ACDB_MEM_CPY(pInput->pBuff + offset, (noOfCdftIndices*sizeof(uint32_t)), pcdft, (noOfCdftIndices*sizeof(uint32_t)));
                              offset += (noOfCdftIndices*sizeof(uint32_t));
                              ++noOfEntriesCopied;
                           }
                           else
                           {
      ...
      {
         AcdbInstanceAudioTblType *pCur;
		 noOfBytesReqPerEntry = ((noOfTableIndices + noOfCdftIndices) * sizeof(uint32_t));
...
int32_t Acdb_Instance_sys_reset(void)
{
   ...
   if (g_pTbl != NULL)
   {
      ...
      while (pCur != NULL)
      {
         AcdbInstanceTopologyType *pCurTop;
   ...
   if (g_pVoiceTblInstance != NULL)
   {
      ...
      while (pCur != NULL)
      {
         ...
         {
            AcdbInstancePrimaryKeyType *pPrimaryCur;
         ...
         if (pDataNode != NULL && pTblNode != NULL)
         {
            if (dataType == ACDB_HEAP_DATA_NOT_FOUND || pDataNode != pTblNode->pDataNode)
            {
               ...
               if (persistData == TRUE &&
                  ...
                  fileIndex < ACDB_MAX_ACDB_FILES)
               {
                  AcdbDeltaDataKeyTypeV2 nKeyToFind;
...
int32_t CreateInstanceDeltaDataKey(AcdbDeltaDataKeyTypeV2 *nKeyToCheck, uint32_t tblID, uint32_t idxCount, uint8_t* pIndices, uint32_t moduleID, uint32_t paramID, uint32_t iid)
{
   uint8_t *buffer = NULL;
   uint32_t offset = 0;
   nKeyToCheck->pIndices = (uint8_t *)ACDB_MALLOC(sizeof(uint32_t)* idxCount);
   if (nKeyToCheck->pIndices == NULL)
   {
      return ACDB_INSUFFICIENTMEMORY;
   }
   buffer = (uint8_t *)malloc((idxCount + 3)*sizeof(uint32_t));
   if (buffer == NULL)
   {
      return ACDB_INSUFFICIENTMEMORY;
   }
   ...
   offset = idxCount*sizeof(uint32_t);
...
int32_t CreateDeltaDataKey(AcdbDeltaDataKeyTypeV2 *nKeyToCheck, uint32_t tblID, uint32_t idxCount, uint8_t* pIndices, uint32_t moduleID, uint32_t paramID)
{
   ...
   buffer = (uint8_t *)malloc((idxCount + 2)*sizeof(uint32_t));
   if (buffer == NULL)
   {
      return ACDB_INSUFFICIENTMEMORY;
   }
