...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_data_mgr.c#15 $
	...
	uint8_t *pDstBuff, uint32_t nDstBuffLen,uint32_t *pDstBytesUsed)
{
	...
	for(i=0 ;i< cdefTbl.nLen;i++)
	{
		uint32_t nMemBytesRequired = 0;
		uint32_t nPaddedBytes = 0;
		AdspCalHdrWithInstanceIDFormatType hdr;
		...
		nMemBytesRequired = sizeof(AdspCalHdrWithInstanceIDFormatType) + cData.nLen + nPaddedBytes;
	...
	uint32_t *pSize)
{
	...
	for(i=0 ;i< cdefTbl.nLen;i++)
	{
		...
		nSizeRequired += (sizeof(AdspCalHdrWithInstanceIDFormatType) + cData.nLen + nPaddedBytes);
	}
	*pSize = nSizeRequired;
	return ACDB_SUCCESS;
}
...
int32_t GetCdftPtrIndex(ContentDefWithInstanceTblType cdefTbl, uintptr_t cdftptr,uint32_t *mpindex)
{
	uint32_t i;
	uint8_t blnFound = 0;
	int32_t result = ACDB_ERROR;
	...
	for(i=0;i<cdefTbl.nLen;i++)
	{
		if(((uintptr_t)&cdefTbl.pCntDef[i]) == cdftptr)
		{
			blnFound = 1;
			*mpindex = i;
			break;
