...
$Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_command.c#70 $
...
#define ACDB_SOFTWARE_VERSION_REVISION 0x00000009
         ...
         if (ACDB_SUCCESS == acdb_delta_data_ioctl(ACDBDELTADATACMD_GET_DELTA_ACDB_DATA_COUNT_FOR_ONE_FILE, (uint8_t *)&i, sizeof(uint32_t), (uint8_t *)&delta_data_count, sizeof(uint32_t)))
         {
            if (delta_data_count > 0)
            {
               if (deltaVersion.majorVersion == 1)
               {
                  ...
                  if (ACDB_SUCCESS == acdb_delta_data_ioctl(ACDBDELTADATACMD_GET_DELTA_ACDB_DATA_FOR_ONE_FILE_V1, (uint8_t *)&i, sizeof(uint32_t), (uint8_t *)deltaFileInstance, sizeof(AcdbCmdDeltaFileDataInstanceV2)*delta_data_count))
                  {
                     uint32_t idxVal = 0;
               ...
               else
               {
                  ...
                  if (ACDB_SUCCESS == acdb_delta_data_ioctl(ACDBDELTADATACMD_GET_DELTA_ACDB_DATA_FOR_ONE_FILE_V0, (uint8_t *)&i, sizeof(uint32_t), (uint8_t *)deltaFileInstance, sizeof(AcdbCmdDeltaFileDataInstance)*delta_data_count))
                  {
                     uint32_t idxVal = 0;
                     for (idxVal = 0; idxVal < delta_data_count; idxVal++)
                     {
                        const uint32_t tblId = *deltaFileInstance[idxVal].pTblId;
						uint32_t tempOffset = 0, length = (*deltaFileInstance[idxVal].pIndicesCount + NON_INSTANCE_CDFT_INDICES_COUNT)*sizeof(uint32_t);
						...
						tempOffset = (*deltaFileInstance[idxVal].pIndicesCount*sizeof(uint32_t));
...
int32_t AcdbCmdSetAudProcInfo(AcdbAudProcTableCmdType *pInput)
{
   ...
   while (remaining_bufLength > sizeof(AudProcTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  ACDB_MEM_CPY(buffer, size, &audProcCmnLookupCmd, AUDPROCTBL_INDICES_COUNT *sizeof(uint32_t));
		  offset += AUDPROCTBL_INDICES_COUNT *sizeof(uint32_t);
...
int32_t AcdbCmdSetAudProcData(AcdbAudProcCmdType *pInput)
{
   ...
   else
   {
      ...
      audcmd.nDeviceSampleRateId = pInput->nDeviceSampleRateId;
      audcmd.nApplicationType = pInput->nApplicationType;
	  ACDB_MEM_CPY(buffer, size, &audcmd, AUDPROCTBL_INDICES_COUNT *sizeof(uint32_t));
	  offset += AUDPROCTBL_INDICES_COUNT *sizeof(uint32_t);
...
int32_t AcdbCmdSetAudStreamInfo(AcdbAudStrmTableV2CmdType *pInput)
{
   ...
   while (remaining_bufLength > sizeof(AudStrmTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  ACDB_MEM_CPY(buffer, size, &audStrmCmnLookupCmd, AUDSTREAMTBL_INDICES_COUNT *sizeof(uint32_t));
		  offset += AUDSTREAMTBL_INDICES_COUNT *sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &oneEntryHeader.nModuleId, sizeof(uint32_t));
		  offset += sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &oneEntryHeader.nParamId, sizeof(uint32_t));
		  offset += sizeof(uint32_t);
...
int32_t AcdbCmdSetAudStreamData(AcdbAudStrmV2CmdType *pInput)
{
   ...
   else
   {
	  ...
	  ACDB_MEM_CPY(buffer, size, &audstrmcmd, AUDSTREAMTBL_INDICES_COUNT *sizeof(uint32_t));
	  offset += AUDSTREAMTBL_INDICES_COUNT *sizeof(uint32_t);
	  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &pInput->nModuleId, sizeof(uint32_t));
	  offset += sizeof(uint32_t);
	  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &pInput->nParamId, sizeof(uint32_t));
   ...
   else
   {
      ...
      for (lut_index = 0; lut_index < (int)lutLength; lut_index++)
      {
         ...
         if (isheapentryfound == 1)
         {
            ...
            extraheap_cdot += sizeof(uint32_t)*cdotTbl.nLen;
         }
         //cdft table parsing
      ...
      if (queryType == TABLE_CMD)// write lut size for the first time
      {
         ...
         if (heapsize > 0)
         {
            for (lut_index = 0; lut_index<(int)lutLength; lut_index++)
            {
               ...
               //reserve space
               if (temp_pointer + (int)cdotTbl.nLen*(int)sizeof(uint32_t) + 4>(datapool_start - (int)sizeof(uint32_t)))//reached max cdot entries
               {
                  continue;
               }
               ACDB_MEM_CPY(pInput->nBufferPointer + temp_pointer, sizeof(cdotTbl.nLen), (void*)(&cdotTbl.nLen), sizeof(cdotTbl.nLen));

               ACDB_MEM_CPY(pInput->nBufferPointer + temp_pointer + 4, cdotTbl.nLen*sizeof(uint32_t), (void*)cdotTbl.pDataOffsets, cdotTbl.nLen*sizeof(uint32_t));
               ...
               if (isheapentryfound == 1)
               {
                  ...
                  temp_pointer += cdotTbl.nLen*sizeof(uint32_t);
...
{
   int32_t result = ACDB_SUCCESS;
   uintptr_t nLookupKey, cdftLookup;
   struct node *lut_cdft = NULL, *lut_cdot = NULL, *lut_cdft_buffer = NULL, *lut_cdot_buffer = NULL, *lut_data_buffer = NULL, *lut_data = NULL, *lut_cdot_new = NULL, *lut_cdft_new = NULL;
   int size_cdft = 0, size_cdot = 0, size_data = 0, lut_cdot_entries = 0, lut_cdft_entries = 0, lut_data_entries = 0;
   ...
   {
      int LookUptble_Length = 0, temp_size = 0, lut_index_count = 0, lut_index = 0, major = 1, minor = 1, ofTbloffset = 0, cvdTbloffset = 0, lutLength = 0;
      int cdft_start = 0, cdot_start = 0, i = 0, j = 0, k = 0, l = 0, m = 0;
      int cdft_offset = 0, cdot_offset = 0, datapool_offset = 0, datapool_start = 0;
      uintptr_t data_Seclookup;
      int lut_cdftfound = 0, lut_cdotfound = 0, calc_data = 0, heapsize = 0;
      ...
      for (lut_index = 0; lut_index < (int)lutLength; lut_index++)
      {
         ...
         if (!lut_cdftfound)
         {
            ...
            }
            {
               ...
               if (!update(calc_cdft*sizeof(uint32_t) * 3 + sizeof(uint32_t) + GetAt(j, &lut_cdft_buffer), j + 1, &lut_cdft_buffer))
               {
                  result = ACDB_INSUFFICIENTMEMORY;
                  goto end;
               }
      ...
      if (queryType == TABLE_CMD)// write lut size for the first time
      {
         ...
         for (k = 0; k < lut_cdft_entries; k++)//fill cdft table in buffer
         {
            ...
            for (i = 0; i < (int)cdefTbl.nLen; i++)
            {
               ...
               cdft_offset += sizeof(iid);
               dataOffset += sizeof(iid);

               ACDB_MEM_CPY(pInput->nBufferPointer + cdft_offset, sizeof(reserved), (void*)(&reserved), sizeof(reserved));
               cdft_offset += sizeof(reserved);
               dataOffset += sizeof(reserved);
...
int32_t AcdbCmdGetVocStream2Info(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      uintptr_t nLookupKey, ncdftLookupKey;
      ...
      if (queryType == TABLE_CMD)// write lut size for the first time
      {
         ...
         if (/*isheapentryfound==1 &&*/ heapsize > 0)
         {
            for (lut_index = 0; lut_index<(int)lutTbl.nLen; lut_index++)
            {
               ...
               ACDB_MEM_CPY(pInput->pBufferPointer + temp_pointer + 4, cdotTbl.nLen*sizeof(uint32_t), (void*)cdotTbl.pDataOffsets, cdotTbl.nLen*sizeof(uint32_t));
...
int32_t AcdbCmdGetVocStreamWithInstanceInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   {
      int LookUptble_Length = 0, temp_size = 0, lut_index = 0, major = 1, minor = 1;//,lutLength=0;
      int cdft_start = 0, cdot_start = 0, i = 0, j = 0, k = 0, l = 0, m = 0, temp_pointer = 0, isheapentryfound = 0, extraheap_cdot = 0;
      int cdft_offset = 0, cdot_offset = 0, datapool_offset = 0, datapool_start = 0;
      int lut_cdftfound = 0, lut_cdotfound = 0, calc_data = 0, heapsize = 0;
      VocProcStatCVDTblHdrFmtType temp_lut_node;
   ...
   uint8_t *pBuff, uint32_t nBuffLen, uint32_t *pBuffBytesFilled)
{
   ...
   else
   {
      ...
      ContentDefTblType cdefTbl;
      ContentDefWithInstanceTblType cdefInstanceTbl;
      ContentDataOffsetsTblType cdotTbl;
      ...
      uint32_t offset = nIdxCount*sizeof(uint32_t);
   ...
   uint8_t *pInBuff, uint32_t nInBuffLen)
{
   ...
   else
   {
      ...
      uint32_t noOfTableIndices = 0;
      ContentDefTblType cdefTbl;
      ContentDefWithInstanceTblType cdeftInstanceTbl;
      ContentDataOffsetsTblType cdotTbl;
      ...
      VocProcDynDataLookupType_UniqueData tempLutDyn;
      VocProcDynDataLookupTblType_UniqueData lutTbl;
      VocProcDynDataLookupTblType_CommonData lutTblCVD;
      VocProcDynDataLookupTblType_offsetData lutoffsetTbl;
      VocProcStatDataLookupType_UniqueData tempLutStat;
      VocProcStatDataLookupTblType_UniqueData lutStatTbl;
      VocProcStatDataLookupTblType_CommonData lutStatTblCVD;
      VocProcStatDataLookupTblType_offsetData lutStatoffsetTbl;
      AcdbTableInfoCVD cvdTblInfo;
      ...
      if (tblId == VOCPROC_DYNAMIC_TBL)
      {
         ...
         ACDB_MEM_CPY(&ofTbloffset, sizeof(uint32_t), &tempLutDyn.nOffTableOffset, sizeof(uint32_t));
         ACDB_MEM_CPY(&cvdtableoffset, sizeof(uint32_t), &tempLutDyn.nCommonTblOffset, sizeof(uint32_t));
         lutTblCVD.nLen = READ_UInt32(cvdTblInfo.tblLutCVDChnk.pData + cvdtableoffset);
         lutTblCVD.pLut = (VocProcDynDataLookupType_CommonData *)(cvdTblInfo.tblLutCVDChnk.pData + cvdtableoffset + sizeof(lutTbl.nLen));
         if (lutTblCVD.nLen <= 0)
         {
      ...
      else if (tblId == VOCPROC_STATIC_TBL)
      {
         ...
         ACDB_MEM_CPY(&ofTbloffset, sizeof(uint32_t), &tempLutStat.nOffTableOffset, sizeof(uint32_t));
         ACDB_MEM_CPY(&cvdtableoffset, sizeof(uint32_t), &tempLutStat.nCommonTblOffset, sizeof(uint32_t));
         lutStatTblCVD.nLen = READ_UInt32(cvdTblInfo.tblLutCVDChnk.pData + cvdtableoffset);
         lutStatTblCVD.pLut = (VocProcStatDataLookupType_CommonData *)(cvdTblInfo.tblLutCVDChnk.pData + cvdtableoffset + sizeof(lutTbl.nLen));
         if (lutStatTblCVD.nLen <= 0)
         {
...
int32_t AcdbCmdSetVP3Data(AcdbVP3CmdType *pInput)
{
   ...
   else
   {
      ...
      if (pInput->nUseCaseId == ACDB_VP3_AUDIO_REC_USECASE)
      {
		 ...
		 uint32_t offset = 0, size= (tblIndicesCount + cdftIndicesCount)*sizeof(uint32_t);
         VP3DevCmdLookupType vp3Devcmd;
		 uint8_t *buffer = (uint8_t *) malloc(size);
		 if(buffer == NULL)
	     {
...
int32_t AcdbCmdGetWdspBootTimeModulesList(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      case TABLE_CMD:
      {
         ...
         ACDB_MEM_CPY(pInput->nBufferPointer + sizeof(uint32_t), pInput->nBufferLength, cdefTbl.pCntDef, cdefTbl.nLen* sizeof(uint32_t));
         pOutput->nBytesUsedInBuffer = sizeof(uint32_t) + (cdefTbl.nLen * sizeof(uint32_t));
         if (ACDB_SUCCESS != result)
         {
            return result;
         }
      ...
      {
         AcdbSizeResponseType *pOutput = (AcdbSizeResponseType *)pOut;
         pOutput->nSize = sizeof(uint32_t) + (cdefTbl.nLen * sizeof(uint32_t)); //cdef chunk contains list of modules.
      }
      break;
      default:
         return  ACDB_ERROR;
...
int32_t AcdbCmdGetWdspDynamicModuleMetaInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   {
      uint32_t index, j, moduleCount = 0;
      uint32_t tblId = WDSP_DYN_MOD_INFO_TBL_ID;
      AcdbTableCmd cmd;
      AcdbTableInfo tblInfo;
      WdspDynamicModuleInfoLookupTblType lutTbl;
      ...
      case TABLE_SIZE_CMD:
      {
         ...
         size += sizeof(uint32_t); //module count
         ...
         for (i = 0; i < cdefTbl.nLen; i++)
         {
            ...
            if (AcdbCmdWdspIsModulePresent(queryType, pIn, moduleID))
            {
               size += cData.nLen + sizeof(uint32_t) + nPaddedBytes;
            }
         }
         pOutput->nSize = size;
      }
...
int32_t AcdbCmdSetAudProcInstanceData(AcdbAudProcInstanceCmdType *pInput)
{
   ...
   else
   {
      ...
      audcmd.nDeviceSampleRateId = pInput->nDeviceSampleRateId;
      audcmd.nApplicationType = pInput->nApplicationType;
	  ACDB_MEM_CPY(buffer, size, &audcmd, AUDPROCTBL_INST_INDICES_COUNT *sizeof(uint32_t));
	  offset += AUDPROCTBL_INST_INDICES_COUNT *sizeof(uint32_t);
	  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &pInput->nModuleId, sizeof(uint32_t));
	  offset += sizeof(uint32_t);
...
int32_t AcdbCmdSetAudProcInstanceInfo(AcdbAudProcTableCmdType *pInput)
{
   ...
   AudProcInstanceTableEntryHeader oneEntryHeader;
   uint8_t  *pOneEntryBuffer = NULL;
   ...
   while (remaining_bufLength > sizeof(AudProcInstanceTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  ACDB_MEM_CPY(buffer, size, &audProcCmnLookupCmd, AUDPROCTBL_INST_INDICES_COUNT *sizeof(uint32_t));
		  offset += AUDPROCTBL_INST_INDICES_COUNT *sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &oneEntryHeader.nModuleId, sizeof(uint32_t));
		  offset += sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &iid, sizeof(uint32_t));
...
int32_t AcdbCmdSetAudStreamInstanceInfo(AcdbAudStrmTableV2CmdType *pInput)
{
   ...
   while (remaining_bufLength > sizeof(AudStrmTableEntryHeader))
   {
      ...
      if (remaining_bufLength >= oneEntryHeader.nParamSize)
	  {
		  ...
		  ACDB_MEM_CPY(buffer, size, &audStrmCmnLookupCmd, AUDSTREAMTBL_INST_INDICES_COUNT *sizeof(uint32_t));
		  offset += AUDSTREAMTBL_INST_INDICES_COUNT *sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &oneEntryHeader.nModuleId, sizeof(uint32_t));
		  offset += sizeof(uint32_t);
		  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &iid, sizeof(uint32_t));
		  offset += sizeof(uint32_t);
...
int32_t AcdbCmdSetAudStreamInstanceData(AcdbAudStrmInstanceCmdType *pInput)
{
   ...
   else
   {
	  ...
	  ACDB_MEM_CPY(buffer, size, &audstrmcmd, AUDSTREAMTBL_INST_INDICES_COUNT *sizeof(uint32_t));
	  offset += AUDSTREAMTBL_INST_INDICES_COUNT *sizeof(uint32_t);
	  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &pInput->nModuleId, sizeof(uint32_t));
	  offset += sizeof(uint32_t);
	  ACDB_MEM_CPY(buffer + offset, sizeof(uint32_t), &iid, sizeof(uint32_t));
...
int32_t AcdbCmdGetPPSequenceInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   {
      AcdbGetTopologyIdRspType topinfo;
      uint8_t* pTopInfo = NULL;
      uint32_t offset = 0, nDeviceID;
      ...
      while (offset <= glbPropInfo.dataInfo.nDataLen)
      {
         ...
         if (topid == topinfo.nTopologyId)
         {
            ...
            case DATA_CMD:
            {
               ...
               pOutput->nBytesUsedInBuffer = size*sizeof(uint32_t) + sizeof(uint32_t);
            }
            break;
            case DATA_SIZE_CMD:
            {
               AcdbSizeResponseType *pOutput = (AcdbSizeResponseType *)pOut;
               pOutput->nSize = size*sizeof(uint32_t) + sizeof(uint32_t);
            }
            break;
            default:
               return ACDB_ERROR;
...
int32_t AcdbCmdGetCodecPPDataInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      AcdbTableCmd cmd;
      AcdbTableInfo tblInfo;
      CodecGainDataLookupTblType lutTbl;
      ContentDefTblType cdefTbl;
      ContentDataOffsetsTblType cdotTbl;
      uintptr_t nLookupKey;
      AcdbCodecGainCmdType codecGaincmd;
      CodecGainDataLookupType tempLut;
...
int32_t AcdbCmdGetCodecPPTableInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      ContentDataOffsetsTblType cdotTbl;
      uintptr_t nLookupKey;
      AcdbCodecGainTableCmdType codecGaincmd;
      CodecGainDataLookupType tempLut;
      uint32_t tblId = CODEC_GAIN_TBL;
      codecGaincmd.nDeviceId = 0;
      codecGaincmd.nGainStep = 0;
...
int32_t AcdbCmdGetCodecPPCmnTableInfo(uint32_t queryType, uint8_t *pIn, uint8_t *pOut)
{
   ...
   else
   {
      ...
      AcdbTableCmd cmd;
      AcdbTableInfo tblInfo;
      CodecCommonDataLookupTblType lutTbl;
      ContentDefTblType cdefTbl;
      ContentDataOffsetsTblType cdotTbl;
      uintptr_t nLookupKey;
      AcdbCodecCommonTableCmdType codecGaincmd;
      CodecCommonTblLookupType tempLut;
...
int32_t AcdbCmdGetCodecGainLevelsInfo(uint32_t queryType, uint8_t * pOutput)
{
   ...
   else
   {
      ...
      case DATA_CMD:
      {
         ...
         ACDB_MEM_CPY(pout->Payload, pout->NoOfentries * 3 * sizeof(uint32_t), glbPropInfo.dataInfo.pData + sizeof(uint32_t), pout->NoOfentries * 3 * sizeof(uint32_t));
      }
      break;
      case DATA_SIZE_CMD:
      {
         AcdbCodecGainLevelsRspType pout;
         AcdbSizeResponseType* p = (AcdbSizeResponseType *)pOutput;
         pout.NoOfentries = READ_UInt32(glbPropInfo.dataInfo.pData);
         p->nSize = (uint32_t)pout.NoOfentries * 3 * sizeof(uint32_t);
      }
      break;
      default:
         return ACDB_ERROR;
...
{
   int32_t result = ACDB_SUCCESS;
   AcdbGroupDeviceListRspType *pout;
   AcdbSizeResponseType* p;
   if (pOut == NULL)
   {
