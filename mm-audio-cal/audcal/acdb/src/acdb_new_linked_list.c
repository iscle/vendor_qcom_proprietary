	...
	)
{
	...
	if (cdft != (uintptr_t)NULL && pTblNode != NULL && ppTopNode != NULL && pParamId != NULL)
	{
		if (pTblNode->pTopologyNode != NULL)
		{
			...
			if (pTblNode->pTopologyNode->pTopHead != NULL)
			{
				AcdbInstanceTopologyType *pCurTopology;
...
int32_t CreateDeltaDataInstanceNodeOnHeap(AcdbDeltaInstanceDataKeyType *pKeyToCreate,
	AcdbDynamicUniqueDataType *pDataNode,
	AcdbDeltaFileInstanceDataType **pDeltaDataNode
	)
{
	int32_t result = ACDB_BADPARM;
	...
	{
		AcdbDeltaInstanceType* pCurTop = NULL;
		AcdbDeltaFileInstanceDataType *pDeltaData = *pDeltaDataNode;
		if (pDeltaData == NULL)
		{
			*pDeltaDataNode = (AcdbDeltaFileInstanceDataType*)ACDB_MALLOC(sizeof(AcdbDeltaFileInstanceDataType));
			pDeltaData = *pDeltaDataNode;

			if (pDeltaData != NULL)
			{
				pDeltaData->pFileHead = (AcdbDeltaInstanceType*)ACDB_MALLOC(sizeof(AcdbDeltaInstanceType));
		...
		{
			pCurTop = (AcdbDeltaInstanceType *)ACDB_MALLOC(sizeof(AcdbDeltaInstanceType));
			if (pCurTop != NULL)
			{
				pCurTop->pKey = pKeyToCreate;
				pCurTop->pDataNode = pDataNode;
	...
	{
		ACDB_DEBUG_LOG ("[ACDB Linked_List ERROR]->[CreateDeltaDataInstanceNodeOnHeap]->NULL Input pointer");
	}
	return result;
}

int32_t CompareDeltaDataInstanceNodeKeys(AcdbDeltaInstanceDataKeyType *pKey1,
	AcdbDeltaInstanceDataKeyType *pKey2
	)
{
...
int32_t FreeDeltaDataInstanceNode(AcdbDeltaInstanceDataKeyType nKeyToCheck,
	AcdbDeltaFileInstanceDataType *pDeltaData
	)
{
	int32_t result = ACDB_BADPARM;
	int32_t memcmpResult;
	...
	if (pDeltaData != NULL)
	{
		AcdbDeltaInstanceType* pPrev =pDeltaData->pFileHead;
		AcdbDeltaInstanceType* pCur = NULL;
		if(pPrev == NULL)
		{
			return ACDB_BADPARM;
		}
		...
		memcmpResult = CompareDeltaDataInstanceNodeKeys(pPrev->pKey, &nKeyToCheck);
		...
		if (memcmpResult != ACDB_SUCCESS)
		{
			while (pCur)
			{
				memcmpResult = CompareDeltaDataInstanceNodeKeys(pCur->pKey, &nKeyToCheck);
				if (memcmpResult == ACDB_SUCCESS)
				{
					break;
				}
...
int32_t UpdateDeltaDataInstanceNodeOnHeap(AcdbDeltaInstanceDataKeyType nKeyToCheck,
	AcdbDynamicUniqueDataType *pDataNode,
	AcdbDeltaFileInstanceDataType *pDeltaData
	)
{
	...
	{
		AcdbDeltaInstanceType* pPrev =pDeltaData->pFileHead;
		AcdbDeltaInstanceType* pCur = pPrev->pNext;
	...
	{
		ACDB_DEBUG_LOG ("[ACDB Linked_List ERROR]->[UpdateDeltaDataInstanceNodeOnHeap]->NULL Input pointer");
	}
	return result;
}
