...
#define CNSS_FW_INIT_DONE		BIT(4)
...
#define CNSS_IS_FW_INIT_DONE(_state) ((_state) & CNSS_FW_INIT_DONE)
			    ...
			    void *ind_cb_data)
{
	...
	case QMI_WLFW_FW_INIT_DONE_IND_V01:
		wsvc_printf_dbg("%s: Received FW INIT Done Ind", __func__);
		pthread_mutex_lock(&gdata.mutex);
		gdata.state |= CNSS_FW_INIT_DONE;
		pthread_cond_signal(&gdata.cond_rsp);
		pthread_mutex_unlock(&gdata.mutex);
		break;
	default:
...
static int wlfw_moselle_init(void)
{
	...
	if (fw_status & QMI_WLFW_FW_INIT_DONE_V01) {
		wsvc_printf_dbg("FW  INIT is already done");
		gdata.state |= CNSS_FW_INIT_DONE;
	}
	...
	pthread_mutex_lock(&gdata.mutex);
	if (!CNSS_IS_FW_INIT_DONE(gdata.state)) {
		wsvc_printf_dbg("Wait for FW INIT done indication");
		pthread_cond_wait(&gdata.cond_rsp, &gdata.mutex);
	}
	pthread_mutex_unlock(&gdata.mutex);
